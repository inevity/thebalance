...
impl<K> AccessTime for DeqNode<KeyDate<K>> {
    ...
    #[inline]
    fn time_to_live(&self) -> Option<std::time::Duration> {
        self.element.entry_info.time_to_live()
    }
}
...
impl<K, V> ValueEntry<K, V> {
    ...
    pub(crate) fn set_time_to_live(&self, time_to_live: Option<std::time::Duration>) {
        self.info.set_time_to_live(time_to_live);
    }
}
