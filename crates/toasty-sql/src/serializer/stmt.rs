...
impl ToSql for &stmt::Limit {
    fn to_sql<P: Params>(self, f: &mut super::Formatter<'_, P>) {
        match &self.offset {
            Some(stmt::Offset::Count(offset)) => {
                fmt!(f, "LIMIT " self.limit " OFFSET " offset);
            }
            Some(stmt::Offset::After(_)) => {
                panic!("Keyset-based pagination not yet supported");
            }
            None => {
...
impl ToSql for &stmt::Returning {
    fn to_sql<P: Params>(self, f: &mut super::Formatter<'_, P>) {
        match self {
            ...
            a => panic!("Unhandled toasty_sql::Returning variant: {:?}\nBacktrace:\n{}", a, Backtrace::capture()),
        }
    }
}
...
impl ToSql for &stmt::Source {
    fn to_sql<P: Params>(self, f: &mut super::Formatter<'_, P>) {
        match self {
            ...
            stmt::Source::Model(model) => {
                // The SQL serializer should only receive lowered queries with Table sources.
                // Model sources need to be lowered to Table sources before SQL serialization.
                panic!(
                    "SQL serializer encountered a Model source (ModelId: {:?}). \
                    Queries must be lowered to convert Model sources to Table sources before SQL serialization. \
                    This typically happens in the Db::exec() method, but custom executors need to handle lowering manually.",
                    model.model
                );
            }
        }
...
impl ToSql for &stmt::UpdateTarget {
    fn to_sql<P: Params>(self, f: &mut super::Formatter<'_, P>) {
        match self {
            ...
            //    let table_id = f.serializer.schema.model(*model_id).table;
            //    let table_name = f.serializer.table_name(table_id);
            //    fmt!(f, table_name);
            //}
        }
