...
impl<M: Model> Select<M> {
    ...
    pub fn limit(&mut self, limit: impl Into<stmt::Expr>) -> &mut Self {
        self.untyped.limit = Some(stmt::Limit {
            limit: limit.into(),
            offset: self.untyped.limit.as_ref().and_then(|l| l.offset.clone()),
        });
        self
    }

    pub fn offset(&mut self, offset: impl Into<stmt::Expr>) -> &mut Self {
        let offset_expr = stmt::Offset::Count(offset.into());
        match &mut self.untyped.limit {
            Some(limit) => {
                limit.offset = Some(offset_expr);
            }
            None => {
                // If no limit is set, we need to create a limit with a large value
                // This is a common pattern in SQL - you can't have OFFSET without LIMIT
                self.untyped.limit = Some(stmt::Limit {
                    limit: stmt::Expr::from(i64::MAX),
