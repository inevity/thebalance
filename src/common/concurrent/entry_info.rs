...
pub(crate) struct EntryInfo<K> {
    ...
    time_to_live: Mutex<Option<Duration>>,
}
...
impl<K> EntryInfo<K> {
    ...
    #[inline]
    pub(crate) fn set_time_to_live(&self, duration: Option<Duration>) {
        *self.time_to_live.lock().expect("lock poisoned") = duration;
    }

    #[inline]
    pub(crate) fn time_to_live(&self) -> Option<Duration> {
        *self.time_to_live.lock().expect("lock poisoned")
    }
}
