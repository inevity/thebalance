    ...
    S: BuildHasher + Clone + Send + Sync + 'static,
{
    ...
    /// Inserts a key-value pair into the cache with a specific time to live.
    ///
    /// If the cache has this key present, the value is updated.
    pub fn insert_with_ttl(&self, key: K, value: V, time_to_live: Duration) {
        let hash = self.base.hash(&key);
        let key = Arc::new(key);
        let (op, now) = self.base.do_insert_with_ttl(key, hash, value, time_to_live);
...
mod tests {
    ...
    use crate::common::time::{Clock, DefaultClock};
    ...
    fn time_to_live() {
        ...
        let (clock, mock) = DefaultClock::mock();
        cache.set_expiration_clock(Some(Arc::new(clock)));
