    ...
    S: BuildHasher + Clone + Send + Sync + 'static,
{
    ...
    #[inline]
    pub(crate) fn do_insert_with_ttl(
        &self,
        key: Arc<K>,
        hash: u64,
        value: V,
        ...
        self.inner
            ...
            .and_modify(|entry| {
                let old_weight = entry.policy_weight();
                *entry = self.new_value_entry_from(value.clone(), ts, weight, entry);
                update_op = Some(WriteOp::UpsertWithTTL {
                    key_hash: KeyHash::new(Arc::clone(&key), hash),
                    value_entry: TrioArc::clone(entry),
                    old_weight,
                    new_weight: weight,
            ...
            .or_insert_with(|| {
                let entry = self.new_value_entry(value.clone(), ts, weight);
                insert_op = Some(WriteOp::UpsertWithTTL {
                    key_hash: KeyHash::new(Arc::clone(&key), hash),
                    value_entry: TrioArc::clone(&entry),
                    old_weight: 0,
                    new_weight: weight,
        ...
        for _ in 0..count {
            match ch.try_recv() {
                ...
                Ok(UpsertWithTTL {
                    ...
                    entry.set_time_to_live(Some(time_to_live));
                    self.handle_upsert(kh, entry, old_weight, new_weight, deqs, &freq, counters)
                }
...
where
    ...
    S: BuildHasher + Clone,
{
    fn set_expiration_clock(&mut self, clock: Arc<dyn Clock>) {
...
fn is_expired_entry_wo(
    ...
    if let Some(ts) = entry.last_modified() {
        ...
        if let Some(ttl) = entry.time_to_live().or(*time_to_live) {
